...
class NLattice:
	...
	def cluster(self):
		size = self.dim
		section = self.grid if self.d == 2 else self.grid[size // 2, :, :]
		base = section[0 : size - 1, 0 : size - 1]
		y_offset = section[1 : size, 0 : size - 1]
		x_offset = section[0 : size - 1, 1 : size]
		nz = count_nonzero(base - x_offset) + count_nonzero(base - y_offset)
		total_pairs = 2 * (size - 1) ** 2
		return 1 - float(nz) / total_pairs
